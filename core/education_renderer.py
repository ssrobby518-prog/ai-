...
「高中程度可讀的多媒體教學文件」：用生活語言、穿插式 QA、
圖片／影片／心智圖，讓 0 基礎成人也能學會科技趨勢。
...
_HIGHSCHOOL_TERM_MAP: dict[str, str] = {
    "Pipeline": "資料生產線",
    "pipeline": "資料生產線",
    "ETL": "把資料抓下來、整理、存起來",
    "Entity": "關鍵人物或公司",
...
def simplify_to_highschool_level(text: str) -> str:
    """將技術詞替換為高中生能懂的生活語言。"""
    result = text
    for tech_term, simple_term in _HIGHSCHOOL_TERM_MAP.items():
...
def generate_learning_assets(topic: str) -> dict[str, str]:
    """為主題生成圖片、影片、Notion 任務、搜尋連結等教學素材。"""
    safe_topic = re.sub(r"[^\w\s\u4e00-\u9fff]", "", topic)[:30].strip()
    query_topic = safe_topic.replace(" ", "+")
    return {
        "image_md": (
            "### 圖解理解\n\n"
            f"![{safe_topic} 示意圖]"
            f"(https://source.unsplash.com/featured/?{query_topic})"
        ),
        "video_md": (
            "### 延伸影片\n\n"
            f"https://www.youtube.com/results?search_query={query_topic}+explained"
        ),
        "notion_task_md": (
            "### Notion 學習任務\n\n"
            "- 今日學到：\n"
...
def generate_mindmap(topic: str) -> str:
    """為主題生成 Mermaid mindmap 語法。"""
    safe_topic = re.sub(r"[^\w\s\u4e00-\u9fff]", "", topic)[:25].strip()
    return (
        "```mermaid\n"
        "mindmap\n"
        f"  root(({safe_topic}))\n"
...
_QA_AFTER_SUMMARY: list[tuple[str, str]] = [
    (
        "這份報告跟我有什麼關係？",
        "這份報告幫你用最短時間掌握今天科技圈發生了什麼事。"
        "就像每天早上聽廣播新聞一樣——"
        "你不需要自己去翻十幾個網站，系統已經幫你整理好重點了。",
    ),
]

_QA_AFTER_PIPELINE: list[tuple[str, str]] = [
    (
        "這個流程圖看起來好複雜，我需要懂嗎？",
        "不用全懂！你只要知道：資料從左邊進來、右邊出去，"
        "就像早餐店的流水線——有人負責煎蛋、有人負責夾土司、有人負責包裝。"
        "每一站各做各的事，最後你拿到的就是一份完整的早餐（報告）。",
    ),
]

_QA_AFTER_METRICS: list[tuple[str, str]] = [
    (
        "系統健康數字是什麼意思？我需要擔心嗎？",
        "這些數字就像學校考試成績一樣——80 分以上算不錯，"
        "50 分以下就要注意了。如果你只是看報告的人，"
        "不需要擔心；如果你是管理者，紅燈時要請工程師檢查。",
    ),
]
...
def _render_qa_block(
    ...
    for q, a in qa_pairs:
        lines.append(f"**Q：{q}**")
        lines.append("")
        lines.append(f"A：{simplify_to_highschool_level(a)}")
        lines.append("")
    return lines


def _make_card_qa(card_title: str) -> list[tuple[str, str]]:
    """為每則新聞卡片生成一組 QA。"""
    short = card_title[:20]
    return [
        (
            f"「{short}」這件事跟一般人有什麼關係？",
            f"你可以把它想成工廠裡某條生產線換了新機器——"
            f"短期內工人要重新學習操作，但長期來看產量會增加。"
            f"「{short}」也是類似的道理：現在看起來只是產業新聞，"
...
def _render_notion_md(
    ...
    # QA：今日結論後
    lines.extend(_render_qa_block(_QA_AFTER_SUMMARY))
    lines.extend(["---", ""])
    ...
    lines.extend([
        ...
        "",
        "Pipeline 就像早餐店的流水線。原始新聞從入口進來，依序經過清洗、分類、評分、深度分析等站點，最終產出結構化的報告。"
        "想像一下：有人負責煎蛋（抓資料）、有人負責夾土司（整理分類）、有人負責包裝（產生報告）。"
        "每個站點專責一項任務，如果某站出錯，不會影響其他站的運作。",
        "",
        "**Q4：為什麼要打分數？分數代表什麼？**",
        "",
        "系統會為每則新聞計算一個綜合分數，就像學校考試一樣——"
        "考量新穎性、實用性、熱度、可行性等維度。只有分數超過門檻（預設 7.0）的項目才會進入深度分析階段。"
        "這個機制叫做「品質關卡」，目的是把有限的運算資源集中在最有價值的內容上。",
        "",
        "**Q5：為什麼會出現「不是新聞的字串」？**",
        "",
    ...
    # QA：流程圖後
    lines.extend(_render_qa_block(_QA_AFTER_PIPELINE))
    lines.extend(["---", ""])
    ...
    # QA：Metrics 後
    lines.extend(_render_qa_block(_QA_AFTER_METRICS))
...
def _render_valid_card_adult(card: EduNewsCard, idx: int) -> list[str]:
    ...
    # 多媒體教學區塊
    assets = generate_learning_assets(card.title_plain)
    lines.extend(["", assets["image_md"], ""])
    lines.extend(["", assets["video_md"], ""])
    lines.extend(["", assets["notion_task_md"]])

    # 心智圖
    lines.extend(["", "#### 心智圖", ""])
    lines.append(generate_mindmap(card.title_plain))
    lines.append("")

    # 每則新聞後的 QA
    lines.extend(_render_qa_block(_make_card_qa(card.title_plain)))
